// erp_v_list_nomenclatures_request.js
...
if(DATA.processName === "syncNomenclatureTransport") {
  ...
  params["$select"] = "Ref_Key,Description,ПодконтрольнаяПродукцияВЕТИС,СрокГодности,ЕдиницаИзмеренияСрокаГодности,ВесЧислитель,ВесЗнаменатель,ЕдиницаИзмерения/Description,ЕдиницаИзмерения_Key"
} else if (DATA.processName === "stockInventory"){
  params["$expand"] = "Parent"
  params["$filter"] = "IsFolder eq false"
  params["$select"] = "Ref_Key,Description,Артикул,DeletionMark,Parent/Description,СрокГодности,ЕдиницаИзмеренияСрокаГодности,ВесЧислитель,ВесЗнаменатель,Code,ПодконтрольнаяПродукцияВЕТИС"
...
return {
  "erp_list_Catalog_Номенклатура": params
};
